...
namespace MCGInventorPlugin.Controllers.SymbolHandler
{
    ...
    public class RibbonController
    {
        ...
        private const string BUTTON_ID            = "id.Button.SymbolHandler";
        private const string BUTTON_DISPLAY       = "Symbol\nHandler";
        private const string BUTTON_TOOLTIP       = "Symbol Handler";
        private const string BUTTON_DESCRIPTION   = "Insert, replace and manage drawing symbols while preserving position and attributes";
        private const string DOCKWIN_ID           = "SymbolHandler.DockableWindow";
        private const string DOCKWIN_TITLE        = "Symbol Handler";
        ...
        /// </summary>
        public SymbolHandlerPanel Panel => _panel;
        ...
        private void EmbedWpfPanel()
        {
            ...
            if (_panel == null)
            {
                ...
                try
                {
                    ...
                    Debug.WriteLine($"{LOG_PREFIX} WPF SymbolHandlerPanel đã tạo.");
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"{LOG_PREFIX} LỖI khi tạo SymbolHandlerPanel: {ex.Message}");
            ...
            var parameters = new HwndSourceParameters("SymbolHandler.WpfHost")
            {
            ...
            // Delayed resize: DockableWindow có thể chưa layout xong lần đầu
            // → GetClientRect trả về kích thước nhỏ → WPF panel bị crop.
            // Sau 500ms DockableWindow đã có kích thước đúng → resize lại.
            ...
            resizeTimer.Tick += (s, ev) =>
            {
                ...
                try
                {
                    ...
                    {
                        RECT r2;
                        if (GetClientRect(new IntPtr(_dockableWindow.HWND), out r2))
                        {
                            int rw = Math.Max(r2.Right - r2.Left, 220);
                            int rh = Math.Max(r2.Bottom - r2.Top, 400);
                            MoveWindow(_hwndSource.Handle, 0, 0, rw, rh, true);
                            _panel?.UpdateLayout();
                            Debug.WriteLine($"{LOG_PREFIX} Delayed resize: {rw}×{rh}px");
                        }
                    }
                }
                catch { }
        ...
        /// Đăng ký tạm thời quanh new SymbolHandlerPanel() để tránh ảnh hưởng toàn bộ AppDomain.
        /// </summary>
        ...
        private void OnSymbolHandlerButtonExecute(NameValueMap context)
        {
